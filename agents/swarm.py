...
def create_crawler_crew(
    ...
    formatting_task = Task(
        description=(
            ...
            "For each extracted item, report the details here using an H3 heading prefixed with an emoji (e.g. '### 📰 Title') WITHOUT any numbering (do NOT write '1.', '2.', etc. in the heading). Resolve and report ONLY the unmerged/unique items here (those that were NOT merged into any canonical story in the deduplication stage). Resolve each unmerged item ID to its full title, entity type, comprehensive summary, and verified source links from the extraction and verification stages. Do NOT include any items here that were duplicates/merged under Section 2 (Deduplicated canonical stories).\n"
